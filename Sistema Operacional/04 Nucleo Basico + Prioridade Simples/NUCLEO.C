...
{
    int nome, estado, prioridade, prior_aux;
    PTR_DESC contexto;
...
void far Escalador()
{
    ...
    while(1)
    {
        ...
        PRIM->prior_aux++; /* Incrementando que ja recebeu + 1 fatia de tempo, independente de estar num trecho do DOS */
        if (!*a.y) /* Houve Interrupcao durante um trecho do DOS? */
        {
            /* Se entrou aqui, nao estava em um trecho do DOS! */
            if (PRIM->prior_aux >= PRIM->prioridade) /* Verificando de ja rodou o numero de fatias pedidas */
            {
                PRIM->prior_aux -= PRIM->prioridade; /* Caso tenha recebido fatia extra, sera considerado aqui. */
