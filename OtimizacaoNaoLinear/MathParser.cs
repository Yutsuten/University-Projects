...
namespace Mathos.Parser
{
    ...
    public class MathParser
    {
        ...
        /// <param name="mathExpression"></param>
        /// <returns></returns>
        public decimal ProgrammaticallyParse(string mathExpression, bool correctExpression = false, bool identifyComments = false)
        {
            if (identifyComments)
            {
        ...
        private decimal Scanner(string _expr)
        {
           ...
           /* foreach (var item in LocalVariables)
            ...
            for (int i = 0; i < _expr.Length; i++)
            {
                ...
                else if ((i + 1) < _expr.Length && (ch == '-' || ch == '+') && Char.IsDigit(_expr[i + 1]) && (i == 0 || OperatorList.IndexOf(_expr[i - 1].ToString()) != -1 || (i > 0 && _expr[i - 1] == '(')))
                {
        ...
        private decimal BasicArithmeticalExpression(List<string> _tokens)
        {
            ...
            else if (_tokens.Count == 2)
            {
                ...
                if (op == "-" || op == "+")
                {
                    String operation = (op == "+" ? "" : "-") + _tokens[1];
                    operation = operation.Replace("+-", "-"); // some basic arithmetical rules
                    operation = operation.Replace("-+", "-");
                    operation = operation.Replace("--", "+");
                    return decimal.Parse(operation, CULTURE_INFO);
                }
                else
                {
